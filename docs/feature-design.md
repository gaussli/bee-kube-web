...
### PersistentVolumeStatusObj - PersistentVolume 观测状态
- phase?: string （状态，取值 Available / Bound / Released / Failed）
- reason?: string （状态原因（通常为失败原因））
- message?: string （状态描述消息）

### PersistentVolumeSource - PersistentVolume 存储后端来源（联合类型，仅可设置其一）
- hostPath?: HostPathVolumeSource （宿主机路径来源，详见 ### HostPathVolumeSource）
- local?: LocalVolumeSource （节点本地存储来源，详见 ### LocalVolumeSource）
- nfs?: NFSVolumeSource （NFS 网络存储来源，详见 ### NFSVolumeSource）
- csi?: CSIVolumeSource （CSI 驱动存储来源，详见 ### CSIVolumeSource）

### HostPathVolumeSource - 宿主机路径来源
- path: string （宿主机上的路径）
- type?: string （路径类型，如 DirectoryOrCreate / Directory / FileOrCreate / File / Socket / CharDevice / BlockDevice）

### LocalVolumeSource - 节点本地存储来源
- path: string （节点上的本地挂载路径）
- fsType?: string （文件系统类型，如 ext4 / xfs）
- nodeAffinity?: VolumeNodeAffinity （节点亲和性；local 类型必须设置，仅允许 required 节点选择）

### NFSVolumeSource - NFS 网络存储来源
- server: string （NFS 服务器地址或主机名）
- path: string （NFS 服务器导出的路径）
- readOnly?: boolean （是否只读挂载，默认 false）

### CSIVolumeSource - CSI 驱动存储来源
...
### VolumeNodeAffinity - 卷节点亲和性
- required?: Record<string, unknown> （强制匹配的节点选择器，须设置 nodeSelectorTerms 限定可调度节点）
