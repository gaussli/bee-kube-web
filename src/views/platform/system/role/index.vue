  ...
  <div class="role-table">
    ...
    <div class="table-header">
      <div class="query-form">
        <div class="query-form-left">
          <BeeSearchInput v-model="searchKey" placeholder="按 ID / 角色名称 / 编码 搜索" />
    ...
    <div class="table-body">
      <el-table v-loading="loading" :data="tableData" height="100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column min-width="150" prop="description">
          <template #header>
            <BeeIconLabel icon="document" label="描述" />
          </template>
          <template #default="{ row }">
            <el-tooltip :content="row.description || '-'" :disabled="!row.description" placement="top">
        ...
        <el-table-column prop="status" width="100">
          ...
          <template #default="{ row }">
            <BeeStatusCell :options="roleStatusConfig" :status="row.status" />
          </template>
        </el-table-column>
            ...
            <el-tooltip v-if="hasPermission('system:role:edit')" content="更多" placement="top">
              <el-dropdown trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item
                      v-if="row.status === 0 && hasPermission('system:role:edit')"
                      @click="handleToggleStatus(row)"
                    >
                      <el-icon><CircleCheck /></el-icon> 启用
                    </el-dropdown-item>
                    <el-dropdown-item
                      v-if="row.status === 1 && hasPermission('system:role:edit')"
                      @click="handleToggleStatus(row)"
                    >
