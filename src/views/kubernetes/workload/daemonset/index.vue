<template>
  <BeePage>
      ...
      <div class="page-body__table">
        ...
        >
          ...
          <BeeTableColumn :width="120">
            <template #default="{ row }">
              <BeeTableCommonCell :label="`${row.numberReady} / ${row.desiredNumberScheduled}`" sublabel="副本数" />
            </template>
          </BeeTableColumn>
          <!-- 更新策略列 -->
          <BeeTableColumn :width="160">
    ...
    >
      <span>
        您确认重启 <strong>{{ selectedRow?.name || '' }}</strong> 守护进程集吗？
      </span>
    </BeeDialog>
    ...
    >
      <span>
        您确认恢复 <strong>{{ selectedRow?.name || '' }}</strong> 守护进程集的更新吗？
      </span>
    </BeeDialog>
    ...
    >
      <span>
        您确认暂停 <strong>{{ selectedRow?.name || '' }}</strong> 守护进程集的更新吗？
      </span>
    </BeeDialog>
...
import { useDaemonSetAction } from './composables/useAction'
import { useDaemonSetFetch } from './composables/useFetch'
import { useDaemonSetPermission } from './composables/usePermission'
import { useDaemonSetTable } from './composables/useTable'
...
// ==================== DaemonSet Composables ====================
const { permissionMap } = useDaemonSetPermission()
const { tableRef, loading, selectedRow, selectedRows, handleSelectionChange } = useDaemonSetTable()
const { queryForm, pageData, daemonSets, fetchDaemonSets } = useDaemonSetFetch(clusterUid, loading)
const {
  searchKey,
  restartDialogConfig,
  resumeDialogConfig,
