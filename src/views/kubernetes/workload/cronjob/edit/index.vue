...
async function handleSubmit() {
  ...
  try { await updateCronJob(clusterId.value, namespace.value, cronjobName.value, { ...formData.value, labels }); ElMessage.success('保存成功'); router.back() } catch {} finally { submitting.value = false }
}
onMounted(() => { loadData() })
</script>

<style lang="scss" scoped>
.cronjob-edit { height: 100%; display: flex; flex-direction: column; }
