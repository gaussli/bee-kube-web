...
import { getJobDetail, updateJob } from '@/api'
...
async function handleSubmit() {
  ...
  try { await updateJob(clusterId.value, namespace.value, jobName.value, { ...formData.value, labels }); ElMessage.success('保存成功'); router.back() } catch {} finally { submitting.value = false }
}
onMounted(() => { loadData() })
</script>

<style lang="scss" scoped>
.job-edit { height: 100%; display: flex; flex-direction: column; }
