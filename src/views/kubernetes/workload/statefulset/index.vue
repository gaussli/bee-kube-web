<template>
  <div class="statefulset-table">
    <div class="page-header">
      <BeeAlert type="info" label="StatefulSet 用于管理有状态应用，为每个 Pod 提供稳定的网络标识和持久存储。" />
      <BeePageTitle :icon="Collection" title="有状态应用" description="管理 Kubernetes StatefulSet 资源。" />
    </div>
    <div class="page-body">
      <div class="table-query">
        ...
        <div class="table-query-right">
          ...
          <BeeButton v-if="hasPermission('kubernetes:workload:statefulset:create')" type="primary" @click="handleCreate"><template #icon><Plus /></template>新增</BeeButton>
        </div>
      </div>
      <div class="table-body">
        <el-table v-loading="loading" :data="tableData" height="100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column min-width="150">
            <template #header><IconLabel :icon="Collection" label="名称" /></template>
          ...
          <el-table-column width="120"><template #header><IconLabel :icon="Cpu" label="副本" /></template><template #default="{ row }"><span :class="getReplicasClass(row)">{{ row.readyReplicas }}/{{ row.replicas }}</span></template></el-table-column>
          <el-table-column min-width="150"><template #header><IconLabel :icon="Link" label="服务名" /></template><template #default="{ row }"><span>{{ row.serviceName }}</span></template></el-table-column>
          ...
          <el-table-column width="200" fixed="right">
            ...
            <template #default="{ row }">
              <el-tooltip content="编辑" placement="top"><el-button v-if="hasPermission('kubernetes:workload:statefulset:edit')" circle :icon="EditPen" size="default" @click="handleEdit(row)" /></el-tooltip>
              <el-tooltip content="详情" placement="top"><el-button circle :icon="View" size="default" @click="handleViewDetail(row)" /></el-tooltip>
              <el-tooltip v-if="hasPermission('kubernetes:workload:statefulset:delete')" content="删除" placement="top"><el-button circle :icon="Delete" size="default" @click="handleDelete(row)" /></el-tooltip>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div class="table-footer">
        <div><BeeButton v-if="hasPermission('kubernetes:workload:statefulset:delete')" type="danger" :disabled="selectedRows.length === 0" @click="handleBatchDelete"><template #icon><Delete /></template>批量删除 ({{ selectedRows.length }})</BeeButton></div>
    ...
    <BeeDialog v-model="deleteDialogVisible" title="确认删除" @confirm="handleConfirmDelete"><div class="dialog-content"><p>确定要删除 StatefulSet <strong>{{ currentTargetRow?.name }}</strong> 吗？</p></div></BeeDialog>
    <BeeDialog v-model="batchDeleteDialogVisible" title="确认删除" @confirm="handleConfirmBatchDelete"><div class="dialog-content"><p>确定要删除选中的 <strong>{{ selectedRows.length }}</strong> 个 StatefulSet 吗？</p><div class="delete-tags"><BeeTag v-for="row in selectedRows" :key="row.id">{{ row.name }}</BeeTag></div></div></BeeDialog>
  </div>
</template>
...
import { Collection, Refresh, Plus, EditPen, Delete, View, FolderOpened, Grid, Cpu, Clock, Link } from '@element-plus/icons-vue'
...
const queryForm = reactive<StatefulSetQueryReq>({ name: undefined, clusterId: kubernetesStore.activeClusterId || undefined, namespace: undefined, page: 1, pageSize: 10 })
...
function getReplicasClass(row: StatefulSetResp) { return row.readyReplicas === row.replicas ? 'replicas-ready' : 'replicas-pending' }
...
function handleSelectionChange(rows: StatefulSetResp[]) { selectedRows.value = rows }
function handleCreate() { router.push({ name: 'kubernetes:workload:statefulset:create' }) }
function handleEdit(row: StatefulSetResp) { router.push({ name: 'kubernetes:workload:statefulset:edit', query: { clusterId: row.clusterId, namespace: row.namespace, name: row.name } }) }
function handleViewDetail(row: StatefulSetResp) { router.push({ name: 'kubernetes:workload:statefulset:detail', query: { clusterId: row.clusterId, namespace: row.namespace, name: row.name } }) }
function handleDelete(row: StatefulSetResp) { currentTargetRow.value = row; deleteDialogVisible.value = true }
async function handleConfirmDelete() { if (!currentTargetRow.value) return; try { await deleteStatefulSet(currentTargetRow.value.clusterId, currentTargetRow.value.namespace, currentTargetRow.value.name); ElMessage.success('删除成功'); deleteDialogVisible.value = false; currentTargetRow.value = null; loadData() } catch {} }
function handleBatchDelete() { batchDeleteDialogVisible.value = true }
async function handleConfirmBatchDelete() { if (selectedRows.value.length === 0) return; const clusterId = selectedRows.value[0].clusterId; const namespace = selectedRows.value[0].namespace; const names = selectedRows.value.map(row => row.name); try { await batchDeleteStatefulSet(clusterId, namespace, names); ElMessage.success(`成功删除 ${names.length} 个 StatefulSet`); batchDeleteDialogVisible.value = false; selectedRows.value = []; loadData() } catch {} }
onMounted(() => { loadData() })
</script>

<style lang="scss" scoped>
.statefulset-table { height: 100%; display: flex; flex-direction: column; }
