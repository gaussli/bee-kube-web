...
import type { NetworkPolicyQueryForm } from '@/types/kubernetes/network/networkpolicy'

import type { ActionItem } from '@/components/business/BeeActionCell/index.vue'

import type { PageEntity } from '@/types'

import { deleteNetworkPolicy, deleteNetworkPolicies } from '@/api/kubernetes/network/networkpolicy'
 ...
 * @param queryForm
 * @param pageData
 * @param fetchNetworkPolicies
 * @param tableRef
 * @param dialogData
 * @param dialogDataList
 */
export function useNetworkPolicyAction<T extends dialogData>(
  clusterUid: Ref<string>,
  permissionMap: Record<string, boolean>,
  queryForm: Reactive<Partial<NetworkPolicyQueryForm>>,
  pageData: Reactive<PageEntity>,
  fetchNetworkPolicies: () => Promise<void>,
  ...
  function handleCreate() {
    router
      .push({ name: KubernetesRouteNames.NetworkPolicy.Create, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleCreateYaml() {
    router
      .push({ name: KubernetesRouteNames.NetworkPolicy.CreateYaml, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleBatchDelete() {
    if (deletableRows.value.length === 0) {
      BeeMessage.warning('选中的网络策略均不可删除')
      return
    }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      await deleteNetworkPolicy(clusterUid.value, namespace, name)
      BeeMessage.success(`成功删除网络策略【${name}】`)
      void fetchNetworkPolicies()
      deleteDialogConfig.visible = false
      dialogData.value = undefined
    } catch (err) {
      console.error('[handleConfirmDelete]', err)
      BeeMessage.error(`删除网络策略【${name}】失败`)
    } finally {
      deleteDialogConfig.loading = false
    }
  }
  ...
  async function handleConfirmBatchDelete() {
    ...
    try {
      ...
      await deleteNetworkPolicies(clusterUid.value, uids)
      BeeMessage.success(`成功删除 ${uids.length} 个网络策略`)
      void fetchNetworkPolicies()
    } catch (err) {
      console.error('[handleConfirmBatchDelete]', err)
      BeeMessage.error('批量删除网络策略失败')
    } finally {
      batchDeleteDialogConfig.loading = false
      batchDeleteDialogConfig.visible = false
