<template>
  <BeePage>
    <!-- 页面 Header -->
    <BeePageHeader v-bind="NETWORKPOLICY_PAGE_META" />
        ...
        >
          ...
          <BeeTableColumn :width="500">
            <template #default="{ row }">
              <NetworkPolicyInfoCell :description="row.description" :name="row.name" :uid="row.uid" />
            </template>
          </BeeTableColumn>
          <!-- 命名空间列 -->
          ...
          <BeeTableColumn :width="160">
            <template #default="{ row }">
              <BeeTableCommonCell :label="String(row.podCount)" sublabel="Pod 数" />
            </template>
          </BeeTableColumn>
          <!-- 创建信息列 -->
          <BeeTableColumn :width="200">
    ...
    >
      <span>
        您确认删除 <strong>{{ selectedRow?.name || '' }}</strong> 网络策略吗？
      </span>
    </BeeDialog>
    ...
    >
      <BeeBatchDeleteDialogContent :delete-data="selectedRows" resource-type="网络策略" />
    </BeeDialog>
  </BeePage>
</template>
...
import { NETWORKPOLICY_PAGE_META } from '@/config/kubernetes/network/networkpolicy'
...
import { useNetworkPolicyTable } from './composables/useTable'

defineOptions({ name: 'NetworkPolicyPage' })
...
// ==================== NetworkPolicy Composables ====================
const { permissionMap } = useNetworkPolicyPermission()
const { tableRef, loading, selectedRow, selectedRows, handleSelectionChange } = useNetworkPolicyTable()
const { queryForm, pageData, networkPolicies, fetchNetworkPolicies } = useNetworkPolicyFetch(clusterUid, loading)
const {
  searchKey,
  deleteDialogConfig,
