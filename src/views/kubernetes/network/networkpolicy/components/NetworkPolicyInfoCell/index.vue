<template>
  <div class="network-policy-info-cell">
    <!-- 左部分：网络策略图标 -->
    <div class="network-policy-info-cell__icon">
      <BeeIcon :name="NETWORKPOLICY_PAGE_META.icon" />
    </div>
    <!-- 右部分：网络策略基础信息（UID、名称、描述） -->
    <div class="network-policy-info-cell__content">
...
/**
 * NetworkPolicyInfoCell 网络策略信息单元格
 * @module views/kubernetes/network/networkpolicy/components/NetworkPolicyInfoCell
 * @description 纯展示单元，接收网络策略字段并对外提供名称复制能力，不发起任何请求
 */
...
defineOptions({ name: 'NetworkPolicyInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 网络策略 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 网络策略名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 网络策略描述，缺省显示 '-' */
    description?: string
  }>(),
...
/**
 * 样式采用 BEM：`__icon` 网络策略图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
 * - 图标配色：`--bee-row-selected-icon-color`，由 BeeTable 选中行注入，缺省为次级文本色
 * - 复制图标默认隐藏，hover 本组件时淡入
 */
.network-policy-info-cell {
  display: flex;
  gap: 8px;
  flex-direction: row;
