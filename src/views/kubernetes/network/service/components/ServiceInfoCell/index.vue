<template>
  <div class="service-info-cell">
    <!-- 左部分：服务图标 -->
    <div class="service-info-cell__icon">
      <BeeIcon :name="SERVICE_PAGE_META.icon" />
    </div>
    <!-- 右部分：服务基础信息（UID、名称、描述） -->
    <div class="service-info-cell__content">
...
/**
 * ServiceInfoCell 服务信息单元格
 * @module views/kubernetes/network/service/components/ServiceInfoCell
 * @description 纯展示单元，接收服务字段并对外提供名称复制能力，不发起任何请求
 */
...
import { SERVICE_PAGE_META } from '@/config/kubernetes/network/service'

defineOptions({ name: 'ServiceInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 服务 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 服务名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 服务描述，缺省显示 '-' */
    description?: string
  }>(),
...
/**
 * 样式采用 BEM：`__icon` 服务图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
