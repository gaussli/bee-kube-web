...
import type { IngressQueryForm } from '@/types/kubernetes/network/ingress'

import type { ActionItem } from '@/components/business/BeeActionCell/index.vue'

import type { PageEntity } from '@/types'

import { deleteIngress, deleteIngresses } from '@/api/kubernetes/network/ingress'
 ...
 * @param queryForm
 * @param pageData
 * @param fetchIngresses
 * @param tableRef
 * @param dialogData
 * @param dialogDataList
 */
export function useIngressAction<T extends dialogData>(
  clusterUid: Ref<string>,
  permissionMap: Record<string, boolean>,
  queryForm: Reactive<Partial<IngressQueryForm>>,
  pageData: Reactive<PageEntity>,
  fetchIngresses: () => Promise<void>,
  ...
  function handleSearch() {
    ...
    queryForm.ingressClassName = searchKey.value
    pageData.page = 1
    void fetchIngresses()
  }
  ...
  function handleReset() {
    ...
    queryForm.ingressClassName = undefined
  ...
  function handleCreate() {
    router.push({ name: KubernetesRouteNames.Ingress.Create, params: { clusterUid: clusterUid.value } }).catch(() => {})
  }
  ...
  function handleCreateYaml() {
    router
      .push({ name: KubernetesRouteNames.Ingress.CreateYaml, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      await deleteIngress(clusterUid.value, namespace, name)
      BeeMessage.success(`成功删除入口【${name}】`)
      void fetchIngresses()
      deleteDialogConfig.visible = false
      dialogData.value = undefined
    } catch (err) {
      console.error('[handleConfirmDelete]', err)
      BeeMessage.error(`删除入口【${name}】失败`)
    } finally {
      deleteDialogConfig.loading = false
    }
  }
  ...
  async function handleConfirmBatchDelete() {
    ...
    try {
      ...
      await deleteIngresses(clusterUid.value, uids)
      BeeMessage.success(`成功删除 ${uids.length} 个入口`)
      void fetchIngresses()
    } catch (err) {
      console.error('[handleConfirmBatchDelete]', err)
      BeeMessage.error('批量删除入口失败')
    } finally {
      batchDeleteDialogConfig.loading = false
      batchDeleteDialogConfig.visible = false
