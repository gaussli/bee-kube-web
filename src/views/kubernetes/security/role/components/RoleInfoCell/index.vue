<template>
  <div class="role-info-cell">
    <!-- 左部分：角色图标 -->
    <div class="role-info-cell__icon">
      <BeeIcon :name="ROLE_PAGE_META.icon" />
    </div>
    <!-- 右部分：角色基础信息（UID、名称、描述） -->
    <div class="role-info-cell__content">
...
/**
 * RoleInfoCell 角色信息单元格
 * @module views/kubernetes/security/role/components/RoleInfoCell
 * @description 纯展示单元，接收角色字段并对外提供名称复制能力，不发起任何请求
 */
...
defineOptions({ name: 'RoleInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 角色 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 角色名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 角色描述，缺省显示 '-' */
    description?: string
  }>(),
...
/**
 * 样式采用 BEM：`__icon` 角色图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
