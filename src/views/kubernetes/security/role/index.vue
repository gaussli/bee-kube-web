<template>
  <BeePage>
    <!-- 页面 Header -->
    <BeePageHeader v-bind="ROLE_PAGE_META" />
        ...
        >
          ...
          <BeeTableColumn :width="500">
            <template #default="{ row }">
              <RoleInfoCell :description="row.description" :name="row.name" :uid="row.uid" />
            </template>
          </BeeTableColumn>
          <!-- 命名空间列 -->
    ...
    >
      <span>
        您确认删除 <strong>{{ selectedRow?.name || '' }}</strong> 角色吗？
      </span>
    </BeeDialog>
    ...
    >
      <BeeBatchDeleteDialogContent :delete-data="selectedRows" resource-type="角色" />
    </BeeDialog>
  </BeePage>
</template>
...
import { ROLE_PAGE_META } from '@/config/kubernetes/security/role'
import { useKubernetesStore } from '@/stores'

import { useNamespaceFetch } from '../../namespace/composables/useFetch'

import RoleInfoCell from './components/RoleInfoCell/index.vue'
import { useRoleAction } from './composables/useAction'
import { useRoleFetch } from './composables/useFetch'
import { useRolePermission } from './composables/usePermission'
import { useRoleTable } from './composables/useTable'
...
// ==================== Role Composables ====================
const { permissionMap } = useRolePermission()
const { tableRef, loading, selectedRow, selectedRows, handleSelectionChange } = useRoleTable()
const { queryForm, pageData, roles, fetchRoles } = useRoleFetch(clusterUid, loading)
const {
  searchKey,
  deleteDialogConfig,
...
} = useRoleAction(clusterUid, permissionMap, queryForm, pageData, fetchRoles, tableRef, selectedRow, selectedRows)
