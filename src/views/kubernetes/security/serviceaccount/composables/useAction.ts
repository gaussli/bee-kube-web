...
import type { ServiceAccountQueryForm } from '@/types/kubernetes/security/serviceaccount'

import type { ActionItem } from '@/components/business/BeeActionCell/index.vue'

import type { PageEntity } from '@/types'

import { deleteServiceAccount, deleteServiceAccounts } from '@/api/kubernetes/security/serviceaccount'
 ...
 * @param queryForm
 * @param pageData
 * @param fetchServiceAccounts
 * @param tableRef
 * @param dialogData
 * @param dialogDataList
 */
export function useServiceAccountAction<T extends dialogData>(
  clusterUid: Ref<string>,
  permissionMap: Record<string, boolean>,
  queryForm: Reactive<Partial<ServiceAccountQueryForm>>,
  ...
  function handleCreate() {
    router
      .push({ name: KubernetesRouteNames.ServiceAccount.Create, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleCreateYaml() {
    router
      .push({ name: KubernetesRouteNames.ServiceAccount.CreateYaml, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleAnnotations(row: T) {
    router
      .push({
        name: KubernetesRouteNames.ServiceAccount.ManageAnnotations,
        params: { clusterUid: clusterUid.value, namespace: row.namespace, name: row.name },
      })
      .catch(() => {})
  }
  ...
  function handleBatchDelete() {
    if (deletableRows.value.length === 0) {
      BeeMessage.warning('选中的服务账号均不可删除')
      return
    }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      await deleteServiceAccount(clusterUid.value, namespace, name)
      BeeMessage.success(`成功删除服务账号【${name}】`)
      void fetchServiceAccounts()
      deleteDialogConfig.visible = false
      dialogData.value = undefined
    } catch (err) {
      console.error('[handleConfirmDelete]', err)
      BeeMessage.error(`删除服务账号【${name}】失败`)
    } finally {
      deleteDialogConfig.loading = false
    }
  }
  ...
  async function handleConfirmBatchDelete() {
    ...
    try {
      ...
      await deleteServiceAccounts(clusterUid.value, uids)
      BeeMessage.success(`成功删除 ${uids.length} 个服务账号`)
      void fetchServiceAccounts()
    } catch (err) {
      console.error('[handleConfirmBatchDelete]', err)
      BeeMessage.error('批量删除服务账号失败')
    } finally {
      batchDeleteDialogConfig.loading = false
      batchDeleteDialogConfig.visible = false
