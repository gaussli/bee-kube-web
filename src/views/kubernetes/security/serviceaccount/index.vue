<template>
  <BeePage>
    <!-- 页面 Header -->
    <BeePageHeader v-bind="SERVICEACCOUNT_PAGE_META" />
        ...
        >
          ...
          <BeeTableColumn :width="500">
            <template #default="{ row }">
              <ServiceAccountInfoCell :description="row.description" :name="row.name" :uid="row.uid" />
            </template>
          </BeeTableColumn>
          <!-- 命名空间列 -->
          ...
          <BeeTableColumn :width="200">
            <template #default="{ row }">
              <BeeTableCommonCell :label="String(row.secretCount)" sublabel="关联 Secret 数" />
            </template>
          </BeeTableColumn>
          <!-- 自动挂载 Token 列 -->
          <BeeTableColumn :width="200">
            <template #default="{ row }">
              <BeeTableCommonCell :label="row.automountServiceAccountToken ? '是' : '否'" sublabel="自动挂载 Token" />
            </template>
          </BeeTableColumn>
          <!-- 创建信息列 -->
          <BeeTableColumn :width="200">
    ...
    >
      <span>
        您确认删除 <strong>{{ selectedRow?.name || '' }}</strong> 服务账号吗？
      </span>
    </BeeDialog>
    ...
    >
      <BeeBatchDeleteDialogContent :delete-data="selectedRows" resource-type="服务账号" />
    </BeeDialog>
  </BeePage>
</template>
...
import { SERVICEACCOUNT_PAGE_META } from '@/config/kubernetes/security/serviceaccount'
import { useKubernetesStore } from '@/stores'

import { useNamespaceFetch } from '../../namespace/composables/useFetch'

import ServiceAccountInfoCell from './components/ServiceAccountInfoCell/index.vue'
import { useServiceAccountAction } from './composables/useAction'
import { useServiceAccountFetch } from './composables/useFetch'
import { useServiceAccountPermission } from './composables/usePermission'
import { useServiceAccountTable } from './composables/useTable'

defineOptions({ name: 'ServiceAccountPage' })
...
// ==================== ServiceAccount Composables ====================
const { permissionMap } = useServiceAccountPermission()
const { tableRef, loading, selectedRow, selectedRows, handleSelectionChange } = useServiceAccountTable()
const { queryForm, pageData, serviceAccounts, fetchServiceAccounts } = useServiceAccountFetch(clusterUid, loading)
const {
  searchKey,
  deleteDialogConfig,
