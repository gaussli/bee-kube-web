<template>
  <div class="service-account-info-cell">
    <!-- 左部分：服务账号图标 -->
    <div class="service-account-info-cell__icon">
      <BeeIcon :name="SERVICEACCOUNT_PAGE_META.icon" />
    </div>
    <!-- 右部分：服务账号基础信息（UID、名称、描述） -->
    <div class="service-account-info-cell__content">
...
/**
 * ServiceAccountInfoCell 服务账号信息单元格
 * @module views/kubernetes/security/serviceaccount/components/ServiceAccountInfoCell
 * @description 纯展示单元，接收服务账号字段并对外提供名称复制能力，不发起任何请求
 */
...
defineOptions({ name: 'ServiceAccountInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 服务账号 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 服务账号名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 服务账号描述，缺省显示 '-' */
    description?: string
  }>(),
...
/**
 * 样式采用 BEM：`__icon` 服务账号图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
 * - 图标配色：`--bee-row-selected-icon-color`，由 BeeTable 选中行注入，缺省为次级文本色
 * - 复制图标默认隐藏，hover 本组件时淡入
 */
.service-account-info-cell {
  display: flex;
  gap: 8px;
  flex-direction: row;
