<template>
  <BeePage>
    <!-- 页面 Header -->
    <BeePageHeader v-bind="CLUSTERROLEBINDING_PAGE_META" />
    ...
    <BeeCard class="page-body">
        ...
        >
          ...
          <BeeTableColumn :width="500">
            <template #default="{ row }">
              <ClusterRoleBindingInfoCell :description="row.description" :name="row.name" :uid="row.uid" />
            </template>
          </BeeTableColumn>
          <!-- 绑定角色列 -->
          <BeeTableColumn :width="200">
            <template #default="{ row }">
              <BeeTableCommonCell :label="row.roleName" :sublabel="`绑定 ${row.roleKind}`" />
            </template>
          </BeeTableColumn>
          <!-- 主体数量列 -->
          <BeeTableColumn :min-width="200">
            <template #default="{ row }">
              <BeeTableCommonCell :label="String(row.subjectCount)" sublabel="主体数量" />
            </template>
          </BeeTableColumn>
          <!-- 创建信息列 -->
          <BeeTableColumn :width="200">
      ...
      <div class="page-body__footer">
        ...
        <BeePagination
          ...
          @change="fetchClusterRoleBindings"
        />
      </div>
    </BeeCard>
    ...
    >
      <span>
        您确认删除 <strong>{{ selectedRow?.name || '' }}</strong> 集群角色绑定吗？
      </span>
    </BeeDialog>
    ...
    >
      <BeeBatchDeleteDialogContent :delete-data="selectedRows" resource-type="集群角色绑定" />
    </BeeDialog>
  </BeePage>
</template>
...
import ClusterRoleBindingInfoCell from './components/ClusterRoleBindingInfoCell/index.vue'
import { useClusterRoleBindingAction } from './composables/useAction'
import { useClusterRoleBindingFetch } from './composables/useFetch'
import { useClusterRoleBindingPermission } from './composables/usePermission'
import { useClusterRoleBindingTable } from './composables/useTable'

defineOptions({ name: 'ClusterRoleBindingPage' })
...
// ==================== ClusterRoleBinding Composables ====================
const { permissionMap } = useClusterRoleBindingPermission()
const { tableRef, loading, selectedRow, selectedRows, handleSelectionChange } = useClusterRoleBindingTable()
const { queryForm, pageData, clusterRoleBindings, fetchClusterRoleBindings } = useClusterRoleBindingFetch(
  clusterUid,
  loading,
)
const {
