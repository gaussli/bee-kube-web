<template>
  <div class="role-binding-info-cell">
    <!-- 左部分：角色绑定图标 -->
    <div class="role-binding-info-cell__icon">
      <BeeIcon :name="ROLEBINDING_PAGE_META.icon" />
    </div>
    <!-- 右部分：角色绑定基础信息（UID、名称、描述） -->
    <div class="role-binding-info-cell__content">
...
/**
 * RoleBindingInfoCell 角色绑定信息单元格
 * @module views/kubernetes/security/rolebinding/components/RoleBindingInfoCell
 * @description 纯展示单元，接收角色绑定字段并对外提供名称复制能力，不发起任何请求
 */
...
defineOptions({ name: 'RoleBindingInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 角色绑定 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 角色绑定名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 角色绑定描述，缺省显示 '-' */
    description?: string
  }>(),
...
/**
 * 样式采用 BEM：`__icon` 角色绑定图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
 * - 图标配色：`--bee-row-selected-icon-color`，由 BeeTable 选中行注入，缺省为次级文本色
 * - 复制图标默认隐藏，hover 本组件时淡入
 */
.role-binding-info-cell {
  display: flex;
  gap: 8px;
  flex-direction: row;
