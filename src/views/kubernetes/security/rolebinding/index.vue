<template>
  <BeePage>
    <!-- 页面 Header -->
    <BeePageHeader v-bind="ROLEBINDING_PAGE_META" />
        ...
        >
          ...
          <BeeTableColumn :width="500">
            <template #default="{ row }">
              <RoleBindingInfoCell :description="row.description" :name="row.name" :uid="row.uid" />
            </template>
          </BeeTableColumn>
          <!-- 命名空间列 -->
    ...
    >
      <span>
        您确认删除 <strong>{{ selectedRow?.name || '' }}</strong> 角色绑定吗？
      </span>
    </BeeDialog>
    ...
    >
      <BeeBatchDeleteDialogContent :delete-data="selectedRows" resource-type="角色绑定" />
    </BeeDialog>
  </BeePage>
</template>
...
import { ROLEBINDING_PAGE_META } from '@/config/kubernetes/security/rolebinding'
import { useKubernetesStore } from '@/stores'

import { useNamespaceFetch } from '../../namespace/composables/useFetch'

import RoleBindingInfoCell from './components/RoleBindingInfoCell/index.vue'
import { useRoleBindingAction } from './composables/useAction'
import { useRoleBindingFetch } from './composables/useFetch'
import { useRoleBindingPermission } from './composables/usePermission'
import { useRoleBindingTable } from './composables/useTable'

defineOptions({ name: 'RoleBindingPage' })
...
// ==================== RoleBinding Composables ====================
const { permissionMap } = useRoleBindingPermission()
const { tableRef, loading, selectedRow, selectedRows, handleSelectionChange } = useRoleBindingTable()
const { queryForm, pageData, roleBindings, fetchRoleBindings } = useRoleBindingFetch(clusterUid, loading)
const {
  searchKey,
  deleteDialogConfig,
