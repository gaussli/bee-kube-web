...
import type { ClusterRoleQueryForm } from '@/types/kubernetes/security/clusterrole'

import type { ActionItem } from '@/components/business/BeeActionCell/index.vue'

import type { PageEntity } from '@/types'

import { deleteClusterRole, deleteClusterRoles } from '@/api/kubernetes/security/clusterrole'
 ...
 * @param queryForm
 * @param pageData
 * @param fetchClusterRoles
 * @param tableRef
 * @param dialogData
 * @param dialogDataList
 */
export function useClusterRoleAction<T extends dialogData>(
  clusterUid: Ref<string>,
  permissionMap: Record<string, boolean>,
  queryForm: Reactive<Partial<ClusterRoleQueryForm>>,
  pageData: Reactive<PageEntity>,
  fetchClusterRoles: () => Promise<void>,
  ...
  function handleCreate() {
    router
      .push({ name: KubernetesRouteNames.ClusterRole.Create, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleCreateYaml() {
    router
      .push({ name: KubernetesRouteNames.ClusterRole.CreateYaml, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleViewDetail(row: T) {
    router
      .push({ name: KubernetesRouteNames.ClusterRole.Detail, params: { clusterUid: clusterUid.value, name: row.name } })
      .catch(() => {})
  }
  ...
  function handleEdit(row: T) {
    router
      .push({ name: KubernetesRouteNames.ClusterRole.Edit, params: { clusterUid: clusterUid.value, name: row.name } })
      .catch(() => {})
  }
  ...
  function handleAnnotations(row: T) {
    router
      .push({
        name: KubernetesRouteNames.ClusterRole.ManageAnnotations,
        params: { clusterUid: clusterUid.value, name: row.name },
      })
      .catch(() => {})
  }
  ...
  function handleBatchDelete() {
    if (deletableRows.value.length === 0) {
      BeeMessage.warning('选中的集群角色均不可删除')
      return
    }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      await deleteClusterRole(clusterUid.value, name)
      BeeMessage.success(`成功删除集群角色【${name}】`)
      void fetchClusterRoles()
    } catch (err) {
      console.error('[handleConfirmDelete]', err)
      BeeMessage.error(`删除集群角色【${name}】失败`)
    } finally {
      deleteDialogConfig.loading = false
      deleteDialogConfig.visible = false
  ...
  async function handleConfirmBatchDelete() {
    ...
    try {
      ...
      await deleteClusterRoles(clusterUid.value, uids)
      BeeMessage.success(`成功删除 ${uids.length} 个集群角色`)
      void fetchClusterRoles()
    } catch (err) {
      console.error('[handleConfirmBatchDelete]', err)
      BeeMessage.error('批量删除集群角色失败')
    } finally {
      batchDeleteDialogConfig.loading = false
      batchDeleteDialogConfig.visible = false
