<template>
  <BeePage>
    <!-- 页面 Header -->
    <BeePageHeader v-bind="CLUSTERROLE_PAGE_META" />
        ...
        >
          ...
          <BeeTableColumn :width="500">
            <template #default="{ row }">
              <ClusterRoleInfoCell :description="row.description" :name="row.name" :uid="row.uid" />
            </template>
          </BeeTableColumn>
          <!-- 规则数列 -->
          <BeeTableColumn :min-width="200">
            <template #default="{ row }">
              <BeeTableCommonCell :label="String(row.ruleCount)" sublabel="策略规则数" />
            </template>
          </BeeTableColumn>
          <!-- 创建信息列 -->
    ...
    >
      <span>
        您确认删除 <strong>{{ selectedRow?.name || '' }}</strong> 集群角色吗？
      </span>
    </BeeDialog>
    ...
    >
      <BeeBatchDeleteDialogContent :delete-data="selectedRows" resource-type="集群角色" />
    </BeeDialog>
  </BeePage>
</template>
...
import { CLUSTERROLE_PAGE_META } from '@/config/kubernetes/security/clusterrole'
import { useKubernetesStore } from '@/stores'

import ClusterRoleInfoCell from './components/ClusterRoleInfoCell/index.vue'
import { useClusterRoleAction } from './composables/useAction'
import { useClusterRoleFetch } from './composables/useFetch'
import { useClusterRolePermission } from './composables/usePermission'
import { useClusterRoleTable } from './composables/useTable'

defineOptions({ name: 'ClusterRolePage' })
...
// ==================== ClusterRole Composables ====================
const { permissionMap } = useClusterRolePermission()
const { tableRef, loading, selectedRow, selectedRows, handleSelectionChange } = useClusterRoleTable()
const { queryForm, pageData, clusterRoles, fetchClusterRoles } = useClusterRoleFetch(clusterUid, loading)
const {
  searchKey,
  deleteDialogConfig,
