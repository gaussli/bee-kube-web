<template>
  <div class="persistent-volume-info-cell">
    <!-- 左部分：持久卷图标 -->
    <div class="persistent-volume-info-cell__icon">
      <BeeIcon :name="PERSISTENTVOLUME_PAGE_META.icon" />
    </div>
    <!-- 右部分：持久卷基础信息（UID、名称、描述） -->
    <div class="persistent-volume-info-cell__content">
...
/**
 * PersistentVolumeInfoCell 持久卷信息单元格
 * @module views/kubernetes/storage/persistentvolume/components/PersistentVolumeInfoCell
 * @description 纯展示单元，接收持久卷字段并对外提供名称复制能力，不发起任何请求
 */
...
import { PERSISTENTVOLUME_PAGE_META } from '@/config/kubernetes/storage/persistentvolume'

defineOptions({ name: 'PersistentVolumeInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 持久卷 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 持久卷名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 持久卷描述，缺省显示 '-' */
    description?: string
  }>(),
...
/**
 * 样式采用 BEM：`__icon` 持久卷图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
 * - 图标配色：`--bee-row-selected-icon-color`，由 BeeTable 选中行注入，缺省为次级文本色
 * - 复制图标默认隐藏，hover 本组件时淡入
 */
.persistent-volume-info-cell {
  display: flex;
  gap: 8px;
  flex-direction: row;
