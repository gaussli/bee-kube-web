...
import type { PersistentVolumeQueryForm } from '@/types/kubernetes/storage/persistentvolume'

import type { ActionItem } from '@/components/business/BeeActionCell/index.vue'

import type { PageEntity } from '@/types'

import { deletePersistentVolume, deletePersistentVolumes } from '@/api/kubernetes/storage/persistentvolume'
 ...
 * @param queryForm
 * @param pageData
 * @param fetchPersistentVolumes
 * @param tableRef
 * @param dialogData
 * @param dialogDataList
 */
export function usePersistentVolumeAction<T extends dialogData>(
  clusterUid: Ref<string>,
  permissionMap: Record<string, boolean>,
  queryForm: Reactive<Partial<PersistentVolumeQueryForm>>,
  pageData: Reactive<PageEntity>,
  fetchPersistentVolumes: () => Promise<void>,
  ...
  function handleCreate() {
    router
      .push({ name: KubernetesRouteNames.PersistentVolume.Create, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleCreateYaml() {
    router
      .push({ name: KubernetesRouteNames.PersistentVolume.CreateYaml, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleBatchDelete() {
    if (deletableRows.value.length === 0) {
      BeeMessage.warning('选中的持久卷均不可删除')
      return
    }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      await deletePersistentVolume(clusterUid.value, name)
      BeeMessage.success(`成功删除持久卷【${name}】`)
      void fetchPersistentVolumes()
    } catch (err) {
      console.error('[handleConfirmDelete]', err)
      BeeMessage.error(`删除持久卷【${name}】失败`)
    } finally {
      deleteDialogConfig.loading = false
      deleteDialogConfig.visible = false
  ...
  async function handleConfirmBatchDelete() {
    ...
    try {
      ...
      await deletePersistentVolumes(clusterUid.value, uids)
      BeeMessage.success(`成功删除 ${uids.length} 个持久卷`)
      void fetchPersistentVolumes()
    } catch (err) {
      console.error('[handleConfirmBatchDelete]', err)
      BeeMessage.error('批量删除持久卷失败')
    } finally {
      batchDeleteDialogConfig.loading = false
      batchDeleteDialogConfig.visible = false
