...
import type { PersistentVolumeClaimQueryForm } from '@/types/kubernetes/storage/persistentvolumeclaim'
...
} from '@/api/kubernetes/storage/persistentvolumeclaim'
 ...
 * @param queryForm
 * @param pageData
 * @param fetchPersistentVolumeClaims
 * @param tableRef
 * @param dialogData
 * @param dialogDataList
 */
export function usePersistentVolumeClaimAction<T extends dialogData>(
  clusterUid: Ref<string>,
  permissionMap: Record<string, boolean>,
  queryForm: Reactive<Partial<PersistentVolumeClaimQueryForm>>,
  pageData: Reactive<PageEntity>,
  fetchPersistentVolumeClaims: () => Promise<void>,
  ...
  /**
   * 搜索
   * @description 关键词同时匹配 UID、名称与存储类名
   */
  function handleSearch() {
    queryForm.uid = searchKey.value
    queryForm.name = searchKey.value
    queryForm.storageClassName = searchKey.value
    pageData.page = 1
  ...
  function handleReset() {
    ...
    queryForm.storageClassName = undefined
  ...
  function handleCreate() {
    router
      .push({ name: KubernetesRouteNames.PersistentVolumeClaim.Create, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleCreateYaml() {
    router
      .push({ name: KubernetesRouteNames.PersistentVolumeClaim.CreateYaml, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleBatchDelete() {
    if (deletableRows.value.length === 0) {
      BeeMessage.warning('选中的持久卷声明均不可删除')
      return
    }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      await deletePersistentVolumeClaim(clusterUid.value, namespace, name)
      BeeMessage.success(`成功删除持久卷声明【${name}】`)
      void fetchPersistentVolumeClaims()
      deleteDialogConfig.visible = false
      dialogData.value = undefined
    } catch (err) {
      console.error('[handleConfirmDelete]', err)
      BeeMessage.error(`删除持久卷声明【${name}】失败`)
    } finally {
      deleteDialogConfig.loading = false
    }
  }
  ...
  async function handleConfirmBatchDelete() {
    ...
    try {
      ...
      await deletePersistentVolumeClaims(clusterUid.value, uids)
      BeeMessage.success(`成功删除 ${uids.length} 个持久卷声明`)
      void fetchPersistentVolumeClaims()
    } catch (err) {
      console.error('[handleConfirmBatchDelete]', err)
      BeeMessage.error('批量删除持久卷声明失败')
    } finally {
      batchDeleteDialogConfig.loading = false
      batchDeleteDialogConfig.visible = false
