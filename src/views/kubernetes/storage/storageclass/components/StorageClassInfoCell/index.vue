<template>
  <div class="storage-class-info-cell">
    <!-- 左部分：存储类图标 -->
    <div class="storage-class-info-cell__icon">
      <BeeIcon :name="STORAGECLASS_PAGE_META.icon" />
    </div>
    <!-- 右部分：存储类基础信息（UID、名称、默认标识、描述） -->
...
/**
 * StorageClassInfoCell 存储类信息单元格
 * @module views/kubernetes/storage/storageclass/components/StorageClassInfoCell
 * @description 纯展示单元，接收存储类字段并对外提供名称复制能力，不发起任何请求
 */
...
import { STORAGECLASS_PAGE_META } from '@/config/kubernetes/storage/storageclass'

defineOptions({ name: 'StorageClassInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 存储类 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 存储类名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 存储类描述，缺省显示 '-' */
    description?: string
    /** 是否为默认存储类，为 true 时在名称后展示「默认」胶囊 */
    isDefault?: boolean
  }>(),
  {
    description: '-',
...
/**
 * 样式采用 BEM：`__icon` 存储类图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
 * - 图标配色：`--bee-row-selected-icon-color`，由 BeeTable 选中行注入，缺省为次级文本色
 * - 复制图标默认隐藏，hover 本组件时淡入
 */
.storage-class-info-cell {
  display: flex;
  gap: 8px;
  flex-direction: row;
