<template>
  <div class="secret-info-cell">
    <!-- 左部分：密钥图标 -->
    <div class="secret-info-cell__icon">
      <BeeIcon :name="SECRET_PAGE_META.icon" />
    </div>
    <!-- 右部分：密钥基础信息（UID、名称、描述） -->
    <div class="secret-info-cell__content">
...
/**
 * SecretInfoCell 密钥信息单元格
 * @module views/kubernetes/config/secret/components/SecretInfoCell
 * @description 纯展示单元，接收密钥字段并对外提供名称复制能力，不发起任何请求
 */
...
import { SECRET_PAGE_META } from '@/config/kubernetes/config/secret'

defineOptions({ name: 'SecretInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 密钥 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 密钥名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 密钥描述，缺省显示 '-' */
    description?: string
  }>(),
...
/**
 * 样式采用 BEM：`__icon` 密钥图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
