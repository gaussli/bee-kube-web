<template>
  <div class="configmap-info-cell">
    <!-- 左部分：配置映射图标 -->
    <div class="configmap-info-cell__icon">
      <BeeIcon :name="CONFIGMAP_PAGE_META.icon" />
    </div>
    <!-- 右部分：配置映射基础信息（UID、名称、描述） -->
    <div class="configmap-info-cell__content">
...
/**
 * ConfigMapInfoCell 配置映射信息单元格
 * @module views/kubernetes/config/configmap/components/ConfigMapInfoCell
 * @description 纯展示单元，接收配置映射字段并对外提供名称复制能力，不发起任何请求
 */
...
defineOptions({ name: 'ConfigMapInfoCell' })

// ==================== Prop ====================
const props = withDefaults(
  defineProps<{
    /** 配置映射 UID，hover UID 胶囊时以 tooltip 展示完整值 */
    uid: string
    /** 配置映射名称，展示于基础信息行并提供复制入口 */
    name: string
    /** 配置映射描述，缺省显示 '-' */
    description?: string
  }>(),
...
/**
 * 样式采用 BEM：`__icon` 配置映射图标、`__content` 信息区（内含 content-top 基础信息行 / content-bottom 描述行）
