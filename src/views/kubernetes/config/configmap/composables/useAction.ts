...
import type { ConfigMapQueryForm } from '@/types/kubernetes/config/configmap'

import type { ActionItem } from '@/components/business/BeeActionCell/index.vue'

import type { PageEntity } from '@/types'

import { deleteConfigMap, deleteConfigMaps } from '@/api/kubernetes/config/configmap'
 ...
 * @param queryForm
 * @param pageData
 * @param fetchConfigMaps
 * @param tableRef
 * @param dialogData
 * @param dialogDataList
 */
export function useConfigMapAction<T extends dialogData>(
  clusterUid: Ref<string>,
  permissionMap: Record<string, boolean>,
  queryForm: Reactive<Partial<ConfigMapQueryForm>>,
  ...
  function handleCreate() {
    router
      .push({ name: KubernetesRouteNames.ConfigMap.Create, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleCreateYaml() {
    router
      .push({ name: KubernetesRouteNames.ConfigMap.CreateYaml, params: { clusterUid: clusterUid.value } })
      .catch(() => {})
  }
  ...
  function handleBatchDelete() {
    if (deletableRows.value.length === 0) {
      BeeMessage.warning('选中的配置映射均不可删除')
      return
    }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      await deleteConfigMap(clusterUid.value, namespace, name)
      BeeMessage.success(`成功删除配置映射【${name}】`)
      void fetchConfigMaps()
      deleteDialogConfig.visible = false
      dialogData.value = undefined
    } catch (err) {
      console.error('[handleConfirmDelete]', err)
      BeeMessage.error(`删除配置映射【${name}】失败`)
    } finally {
      deleteDialogConfig.loading = false
    }
  }
  ...
  async function handleConfirmBatchDelete() {
    ...
    try {
      ...
      await deleteConfigMaps(clusterUid.value, uids)
      BeeMessage.success(`成功删除 ${uids.length} 个配置映射`)
      void fetchConfigMaps()
    } catch (err) {
      console.error('[handleConfirmBatchDelete]', err)
      BeeMessage.error('批量删除配置映射失败')
    } finally {
      batchDeleteDialogConfig.loading = false
      batchDeleteDialogConfig.visible = false
